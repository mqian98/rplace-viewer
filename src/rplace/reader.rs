...
pub struct RedditPlaceDataIterator {
    file_path: String,
    iter: DeserializeRecordsIntoIter<File, RedditPlaceDataPoint>,
}

impl fmt::Debug for RedditPlaceDataIterator {
...
impl RedditPlaceDataIterator {
    pub fn new(file_path: &str) -> Option<RedditPlaceDataIterator> {
        match File::open(file_path) {
            Ok(f) => {
                let rdr = csv::Reader::from_reader(f);
                let iter = rdr.into_deserialize();
                Some(RedditPlaceDataIterator {
...
impl Iterator for RedditPlaceDataIterator {
