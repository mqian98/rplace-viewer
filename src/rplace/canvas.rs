...
impl Canvas {
    ...
    fn load_pixels(reader: RPlaceDataReader) -> Vec<Vec<PixelColor>> {
        // TODO: determine number of pixels for width and height by timestamp instead of hardcoding
        let mut pixels = vec![vec![PixelColor::Black; 1000]; 1000];

        // TODO: make limit an optional parameter
        let limit = 100000;
        for record in reader.into_iter().take(limit) {
            pixels[record.coordinate.y as usize][record.coordinate.x as usize] = record.color;            
        }
        println!("Size of pixels {:?} {:?} {:?}", std::mem::size_of_val(&pixels[0][0]), pixels[0].capacity(),  pixels.capacity());
        return pixels;
    }
}
